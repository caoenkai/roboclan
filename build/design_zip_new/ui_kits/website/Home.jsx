...
(function () {
  ...
  function Home({ onOpenCategory, onOpen, onAdd, compare, onNews, onQuote, onOpenGuides, onOpenPost }) {
    ...
    // 新闻滚动：用 JS 原生 scrollLeft，不用 CSS transform（避免 iOS 超宽图层变空白）
    const tkViewRef = React.useRef(null);
    React.useEffect(() => {
      const view = tkViewRef.current; if (!view) return;
      ...
      const tick = (now) => {
        ...
        if (!paused && half > 1) {
          let x = view.scrollLeft + speed * dt;
          if (x >= half) x -= half;
          view.scrollLeft = x;
        }
      ...
      raf = requestAnimationFrame(tick);
      const enter = () => { paused = true; }, leave = () => { paused = false; last = 0; };
      view.addEventListener("mouseenter", enter); view.addEventListener("mouseleave", leave);
      return () => { cancelAnimationFrame(raf); view.removeEventListener("mouseenter", enter); view.removeEventListener("mouseleave", leave); };
    }, [news]);
    const counts = {};
        ...
        {(() => {
          ...
          return (
            <div className="rc2-ticker"><div className="rc2-tkin">
              ...
              <div className="rc2-tkview" ref={tkViewRef}><div className="rc2-tktrack">
